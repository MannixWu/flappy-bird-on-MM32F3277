...
//------------------------------------------------------------------------------------------------------------------
// @brief		rgb颜色转换为uint16
// @BY MAINNIX_WU				2021/5/7
// @param		colorR			R通道数值
// @param		colorG			G通道数值
// @param		colorB			B通道数值
// @return	uint16			对应的显示器能读取的uint16颜色数值	
// @since		v1.0
// Sample usage:				RGBtoU16(255,255,255);	//白色
// @note		RGB取值范围为0~255
//-------------------------------------------------------------------------------------------------------------------

// r 32 g 64 b 32
uint16 RGBtoU16(int colorR,int colorG, int colorB){
	return (colorR/8)*32*64+(colorG/4)*32+(colorB/8);
}

uint16 toColor(int r,int g,int b){
	return (r/8)*32*64+(g/4)*32+(b/8);
}

uint16 grayToU16(uint8 temp){
	int color;
			color=(0x001f&((temp)>>3))<<11;
			color=color|(((0x003f)&((temp)>>2))<<5);
			color=color|(0x001f&((temp)>>3));
...
// @param		x					x坐标
// @param		y					y坐标
// @param		size			点的半径(实际宽度为size*2+1)
// @param		color			颜色设置
// @return		void
// @since		v1.0
// Sample usage:				drawPointL(10, 20, 2, RED);// 在(10,20)处绘制一个大小为5的红色点
//-------------------------------------------------------------------------------------------------------------------
void drawPointL(int x, int y, int size, uint16 color)
{
	uint16 i,j;
	x=(x<size)?size:x;
	x=(x>IPS114_X_MAX-1-size)?(IPS114_X_MAX-1-size):x;
	y=(y<size)?size:y;
	y=(y>IPS114_Y_MAX-1-size)?(IPS114_Y_MAX-1-size):y;
	size = (size<0)?0:size;
	size = (size>60)?60:size;
	ips114_set_region(x-size,y-size,x+size,y+size);
	for(i=0;i<=size*2;i++)
	{
		for (j=0;j<=size*2;j++)
		{
...
// @return		void
// @since		v1.0
// Sample usage:				drawRect(20,30,50,70,YELLOW);// 从(20,30)到(50,70)处绘制一个黄色的矩形
//-------------------------------------------------------------------------------------------------------------------
void drawRect(int x1, int y1, int x2, int y2, int color)
{
	uint16 i,j;
	x1=between(x1,0,IPS114_X_MAX-1);
	y1=between(y1,0,IPS114_Y_MAX-1);
	x2=between(x2,0,IPS114_X_MAX-1);
	y2=between(y2,0,IPS114_Y_MAX-1);
	if(x1>x2){
		x1=x1^x2;
		x2=x1^x2;
		x1=x1^x2;
...
// @return		void
// @since		v1.0
// Sample usage:				drawLine(0,0,100,100,RED);// 从(0,0)到(100,100)处绘制一条红色直线
//-------------------------------------------------------------------------------------------------------------------
void drawLine(int x1, int y1, int x2, int y2, int color)
{
	...
	for(int i = 0; i < distance;i++){
				//ips114_drawpoint(x1+(x2-x1)*i/distance, y1+(y2-y1)*i/distance, color);
		
				drawPointL(x1+(x2-x1)*i/distance, y1+(y2-y1)*i/distance,0, color);
	}
}
void drawCross(int x,int y,int size, int color){
			drawRect( between(x-size,0,IPS114_X_MAX-1), between(y,0,IPS114_Y_MAX-1), between(x+size,0,IPS114_X_MAX-1), between(y,0,IPS114_Y_MAX-1),color);
			drawRect( between(x,0,IPS114_X_MAX-1), between(y-size,0,IPS114_Y_MAX-1), between(x,0,IPS114_X_MAX-1), between(y+size,0,IPS114_Y_MAX-1),color);
}
//-------------------------------------------------------------------------------------------------------------------
// @brief		rgb图像二维数组液晶显示函数
// @param		*p				图像数组地址
// @param		width			图像宽度
// @param		height			图像高度
// @return		void
// @since		v1.0
// Sample usage:				ips114_displayimage032(mt9v03x_csi_image[0], MT9V03X_CSI_W, MT9V03X_CSI_H)//显示灰度摄像头 图像
// @note		测试用不要乱用，这个函数没啥用
//-------------------------------------------------------------------------------------------------------------------
void ips114_displayimageRGB(uint16 *p, uint16 width, uint16 height) 
{
	uint32 i,j;
