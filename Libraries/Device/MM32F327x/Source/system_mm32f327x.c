...
static void DELAY_xUs(u32 nus)
{
    u32 temp;
    SysTick->LOAD = nus * 8;                                                    //time count for 1us
    SysTick->VAL = 0x00;                                                        //clear counter
    SysTick->CTRL |= SysTick_CTRL_ENABLE_Msk ;                                  //start discrease
    do {
        temp = SysTick->CTRL;
...
//    s32 i;
    ...
    u8 plln, pllm;
//    RCC->ICSCR &= ~(RCC_ICSCR_HSI_CAL_SFT | RCC_ICSCR_HSI_CAL_SEL);
//    RCC->ICSCR |= (0x280 << RCC_ICSCR_HSI_CAL_SFT_Pos );
//    RCC->ICSCR |= (0x1F << RCC_ICSCR_HSI_CAL_SEL_Pos );
    //SYSCLK, HCLK, PCLK2 and PCLK1 configuration ---------------------------
    //Enable HSE
    RCC->CR |= ((u32)RCC_CR_HSEON);
//    for(j = 0; j < 10000; j++) {
